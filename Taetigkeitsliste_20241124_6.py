...
def create_task_list(year, month, tasks, first_name, last_name, mitarbeiternummer, wochenarbeitszeit, resturlaub):
    ...
    # Berechnung der Sollarbeitszeit im Monat (8 Stunden pro Arbeitstag, keine Wochenenden und Feiertage)
    total_soll_minutes = 0  # Sollarbeitszeit in Minuten
    for day in range(1, days_in_month + 1):
        weekday_index = calendar.weekday(year, month, day)
        current_date = date(year, month, day)
        
        # Prüfen, ob das aktuelle Datum ein Feiertag oder Wochenende ist
        if weekday_index < 5 and current_date not in public_holidays_bayern.values():  # Montag bis Freitag und kein Feiertag
            total_soll_minutes += 8 * 60  # 8 Stunden pro Arbeitstag in Minuten
    
    # Umrechnung der Sollarbeitszeit in Stunden und Minuten
    total_soll_hours = total_soll_minutes // 60
    total_soll_minutes = total_soll_minutes % 60
