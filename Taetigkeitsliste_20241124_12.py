...
def create_task_list(year, month, tasks, first_name, last_name, mitarbeiternummer, wochenarbeitszeit, resturlaub):
    ...
    for day in range(1, days_in_month + 1):
        ...
        if holiday_name:  # Feiertag
            ...
            # Sicherstellen, dass pro Tag mindestens 2 und maximal 3 Aufgaben eingetragen werden
            task_entry = ', '.join(random.sample(tasks, random.randint(2, 3)))  # Ändern hier auf 2 bis 3 Aufgaben
...
# Tasks aus Excel laden
def load_tasks_from_excel(file_path):
    df = pd.read_excel(file_path)
    return df['Task'].dropna().tolist()

# Anzeige und Auswahl der Excel-Dateien im aktuellen Verzeichnis
def choose_excel_file():
    ...
    print("\nExcel-Dateien mit den Projekten und Tätikeiten an denen du gearbeitet hast:")
    for idx, file in enumerate(xlsx_files, 1):
        print(f"{idx}: {file}")
    
    while True:
        try:
            choice = int(input("\nWähle die Nummer der gewünschten Datei: "))
...
if not excel_file:
    print("Kein Task-Dokument ausgewählt. Das Programm wird beendet.")
    exit()
...
tasks = load_tasks_from_excel(excel_file)

# Funktion aufrufen
create_task_list(year, month, tasks, first_name, last_name, Mitarbeiternummer, Wochenarbeitszeit, Resturlaub)

current_date = datetime.now().strftime("%d.%m.%Y")  # Aktuelles Datum erneut abrufen
